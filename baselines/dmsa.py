...
class DMSANet(nn.Module):
    def __init__(self, nChan=22, nTime=1000, nClass=4, dropoutP=0.25, *args, **kwargs):
        ...
        pooled_time = (nTime - self.poolkernel) // self.poolstride + 1
        if pooled_time < 1:
            raise ValueError("nTime is too short for the configured pooling layer")
        self.FC = nn.Sequential(
            LinearWithConstraint(self.totalChannels * self.D * pooled_time, nClass, max_norm=0.5),
            nn.LogSoftmax(dim=1)
        )
