...
- `run_massanet.py`: leakage-safe adapter for the 2026 MASSANet model. It imports a separately cloned upstream repository and runs the paper's three-channel LOSO protocol on IV-2b or OpenBMI without copying MASSANet source into this repository.
...
resamples to 250 Hz, and caches fixed 8--30 Hz four-second trials before 54-fold
LOSO training:
...
python baselines/run_massanet.py \
  ...
  --massanet-root /root/autodl-tmp/MASSANet \
  --output outputs/massanet_openbmi_seed42
```
...
parameter count. `summary.json` reports subject-wise mean and sample standard
deviation and marks whether the full LOSO set is complete.
