...
def load_preprocess_data(data, debug, lowcut, highcut, w0, Q, anti_drift, class_count, cutoff, axis, fs,T):
    ...
    if debug:
        ...
        data_notch_filtered = gumpy.signal.notch(data_loaded.raw_data, cutoff, axis, fs=fs, Q=Q)
        data_hp_filtered = gumpy.signal.butter_highpass(data_notch_filtered, anti_drift, axis, fs=fs)
        data_bp_filtered = gumpy.signal.butter_bandpass(data_hp_filtered, lowcut, highcut, axis, fs=fs)
...
def load_raw(dataset):
    raise RuntimeError('Legacy bulk download is disabled because it overwrites files. Use tools/datasets.py with an explicit URL and new output path.')
